...
export default function CreateTestCoachPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await createTestCoach(email, password);
      toast.success('Test coach account created successfully!');
      router.push('/login');
    } catch (error) {
      console.error('Error:', error);
      toast.error('Failed to create test coach account');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="max-w-md w-full space-y-8">
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          <div>
            ...
            >
              {loading ? 'Creating...' : 'Create Test Coach Account'}
            </button>
          </div>
        </form>
      </div>
